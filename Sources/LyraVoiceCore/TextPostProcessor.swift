...
public enum TextPostProcessor {
    ...
    // MARK: - Вводные слова (грамматическая пунктуация)

    /// Расставляет запятые после вводных слов в начале предложения. Это детерминированная
    /// часть «грамотной пунктуации» в «Быстрой» полировке: whisper часто не ставит запятую
    /// после вводного («Конечно сделаю» → «Конечно, сделаю»). Берём ТОЛЬКО слова, которые в
    /// начале фразы обособляются практически всегда — чтобы не наставить ошибочных запятых
    /// там, где слово выступает членом предложения (поэтому здесь нет «однако», «значит»,
    /// «правда» и т.п. — они двусмысленны).
    ///
    /// Срабатывает в начале текста и после `.`/`!`/`?`/`\n`. Не трогает слово, если за ним
    /// уже стоит запятая/двоеточие/тире, и пропускает случай, когда дальше нет текста.
    public static func insertIntroductoryCommas(_ input: String) -> String {
        // Многословные — первыми, чтобы «к сожалению» не разрезалось на «к» + «сожалению».
        let introducers = [
            "во-первых", "во-вторых", "в-третьих", "в-четвёртых", "в-четвертых",
            "к сожалению", "к счастью", "к удивлению", "к примеру",
            "по-моему", "по-видимому", "по-твоему", "по-вашему",
            "кстати", "конечно", "разумеется", "безусловно", "несомненно",
            "наверное", "наверно", "возможно", "вероятно", "пожалуй",
            "например", "впрочем", "итак", "следовательно", "в общем",
            "кроме того", "более того", "таким образом", "с другой стороны"
        ]
        var text = input
        for word in introducers {
            let escaped = NSRegularExpression.escapedPattern(for: word)
            // Группа 1 — начало строки / знак конца предложения + пробелы; группа 2 — само
            // вводное (захватываем, чтобы СОХРАНИТЬ оригинальный регистр «Конечно»/«конечно»);
            // далее НЕ запятая/двоеточие/тире (lookahead) и обязательно ещё буква (есть текст).
            // Финальный lookahead исключает частицы/союзы («же», «бы», «ли», «и», «а», «но»):
            // «конечно же» обособляется ПОСЛЕ «же», а не после «конечно», и т.п.
            let pattern = "(?i)(^|[.!?\\n]\\s*)(\(escaped))(?![,.:;!?…—–-])\\s+(?=[А-Яа-яЁёA-Za-z])(?!(?:же|ж|бы|б|ли|и|а|но)\\b)"
    ...
    private static func splitSentences(_ input: String) -> [String] {
        ...
        // Не считать десятичную дробь / номер версии («1.00», «3.14», «v1.2») концом
        // предложения: если фрагмент кончается на цифру+точку, а следующий начинается с
        // цифры — склеиваем обратно. Иначе «до 1.00 августа» рвалось на «до 1.» + «00
        // августа» и при сборке получалось «до 1. 00 августа».
        var merged: [String] = []
        for sentence in sentences {
            if let last = merged.last,
               last.range(of: #"\d\.$"#, options: .regularExpression) != nil,
               sentence.first?.isNumber == true {
        ...
        return merged.isEmpty ? [input] : merged
    }
}
